...
data class ValueProvider(
    ...
    companion object Default {
        ...
        fun localVarName(n: Int): String = n.toString(36)

        /** Локальная переменная */
        fun localVar(n: Int): Variable = Variable(localVarName(n), Variable.Scope.LINE)
        ...
        /** Переменная, хранящая таблицу виртуальных методов для класса */
        fun vtable(classAddress: Value) = Variable("v$classAddress")
        ...
        fun newInstance(context: TranslationContext, variable: Variable?): Variable {
            ...
            context.addOperation(JustOperation("call_function", mapOf(
                "function_name" to TextValue("NEW"),
                "args" to MapValue(mapOf(
                    TextValue("res") to variable
                ))
            )))
            return variable
        }
